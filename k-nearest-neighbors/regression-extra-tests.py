...
#! Same file as example code from the book but with some added notes and playing around with the model
"""
...
class RegressionTest(object):
  ...
  def load_csv_file(self, csv_file, limit=None):
    ...
    #! Note: idea here that after ther csv is loaded, the appraised value column is assigned to the variable values, and after that it is dropped from the loaded csv. Following that, the df houses is normalized with absolute mean errror in mind (max-min instead of std) and finally only the lat,long,sqftlot is kept for self.houses.
    """
